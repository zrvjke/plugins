/**
 * Lampa plugin: Hide Details Noise (v1.5.2)
 * - Фильмы: скрыть HH:MM в строке деталей.
 * - Сериалы: скрыть Сезоны/Серии + "Следующая … / Осталось дней: N".
 * - Полностью скрыть секцию комментариев между "Актёры" и "Рекомендации".
 * ES5, ASCII-safe, устойчиво к перерисовкам.
 */
...
(function () {
  ...
  var RE_ACTORS = /^(?:\u0410\u043a\u0442\u0435\u0440\u044b|\u0410\u043a\u0442\u0451\u0440\u044b|actors?|cast)$/i;
  ...
  function looksLikeComments(el){
    if (!el) return false;
    // признаки: классы/ид с "comment", внутри есть заголовок "Комментарии" или кнопка-плитка с плюсом
    ...
    // карточки-комментарии часто содержат иконку лайка/аккаунта; минимальная эвристика:
    var plusTile = el.querySelector('button,div,span');
    if (plusTile && textOf(plusTile)==='+') return true;
    // много коротких карточек — тоже вероятно
    return false;
  }
  function killCommentsBetweenActorsAndRecs(root){
    ...
    if (!hRecs) {
      // запасной путь: снести любые явные comments-контейнеры
    ...
    // идём влево (к предыдущим соседям) и скрываем всё, что похоже на комментарии,
    // пока не наткнёмся на секцию актёров
    var cur = secRecs.previousElementSibling, guard=0;
    while (cur && guard++<50){
      // остановиться, если это актёры
      ...
      // скрываем, если есть признаки комментариев или нет ничего содержательного кроме коротких карточек
      if (looksLikeComments(cur) || cur.querySelector('[class*="comment"],[id*="comment"]')) {
        hideNode(cur);
        cur = cur.previousElementSibling;
        continue;
      }
      // если это просто пустой/служебный отступ — тоже прячем
      if (textOf(cur)==='' && cur.children && cur.children.length<=1) { hideNode(cur); cur=cur.previousElementSibling; continue; }
      // иначе дошли до другой секции — прекращаем
      break;
    }
  }
  ...
  function ensureRootObserver(){
    ...
    var pend=false, mo=new MutationObserver(function(){
      ...
      setTimeout(function(){
        ...
        // на всякий: если вдруг классы с "comment" появились — схлопнем CSS-ом
        injectCssOnce();
      },50);
    });
