(function () {
    ...
    // Динамическое добавление секции настроек с проверкой DOM
                    ...
                    if (e.name === 'settings') {
                        ...
                        var checkContainer = setInterval(function () {
                            ...
                            if (settingsContainer.length) {
                                ...
                                console.log('Settings container not yet found, retrying... Available elements:', $('body').find('*').map(function() { return this.className; }).get().join(', '));
                            }
                        }, 100); // Проверка каждые 100 мс
                    }
                });
            }, 500); // Начальная задержка 0.5 секунды
        }
    });
