(function () {
    ...
    // Кэш для рейтингов (чтобы не запрашивать повторно)
    var ratingsCache = {};

    // Стили иконок (можно вынести в CSS Lampa)
    var icons = {
    ...
    // Парсинг Rotten Tomatoes напрямую (fallback)
    ...
    // Получение рейтингов (OMDb + RT fallback)
    function fetchRatings(imdbId, title, callback) {
        ...
        Lampa.Network.get(url, {}, function (json) {
            if (!json || json.Error) {
                scrapeRT(title, callback); // Fallback
            } else {
                var result = {
    ...
    Lampa.Listener.follow('full', function (e) {
        ...
        hideTMDbRating(e.body); // Скрываем TMDb, если включено
